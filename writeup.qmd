...
Books.csv i Column Description\
--- ------ ------------------------------ 0 ISBN unique identifier for each book 1 Book-Title title of the book 2 Book-Author author of the book 3 Year-Of-Publication year the book was published 4 Publisher company that published the book 5 Image-URL-S URL to a small cover image 6 Image-URL-M URL to a medium cover image 7 Image-URL-L URL to a large cover image

Ratings.csv i Column Description --- ------ ------------------- 0 User-ID annonymous unique id for each user 1 ISBN unique identifier for each book 2 Book-Rating rating given to book from User_ID

Link to datasets: https://www.kaggle.com/datasets/arashnic/book-recommendation-dataset/data
...
\- Computed matrices only from books present in the ratings Part 2: Build the Recommendation System 3. Created CBF Model - Combined title + author + publisher into a “text” field - Tokenized using TfidVectorizer - Computed cosine similarity for any selected ISBN 4. Created CF Model - Built full trainset - Used KNNBasic (item-based, cosine) - Extracted neighbor ISBNs for any given book 5. Created Hybrid Model - Normalized similarity vectors - Weighted the contributions of CBF and CF - Returned top 10 recommendations with their hybrid scores

Although I wanted to create a hybrid system that recommends books based on metadata features and user ratings, I created all three models as to confirm that the hybrid model produced the best recommendations. 1. Content-Based Filtering (CBF): Recommends similar books based on book metadata features including title, author, and publisher. 2. Collaborating Filtering (CF): recommends books based on similarity of user rating patterns. For this model, I used Surprise’s KNNBasic model with item-based CF, cosine similarity, and full trainset fitting. 3. Hybrid Model: Hybrid systems are widely used in recommendation systems. I combined both CBF and CF models to give recommendations based on both personal taste and similarity. The model computes a similarity score from CBF and CF, then blends those scores using a parameter (alpha), which controls the balance between the two systems. I chose alpha = 0.6 so the content features have slightly more weight than rating similarities.
...
AI was used to narrow down a list of 709 items that was eventually removed from the project. No AI was used in the final code.